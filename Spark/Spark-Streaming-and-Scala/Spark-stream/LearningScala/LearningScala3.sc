object LearningScala3 {
  ...
  def toupper(x: String): String = {x.toUpperCase}//> toupper: (x: String)String
  val foo_upper = toupper("foo")                  //> foo_upper  : String = FOO
  println(foo_upper)                              //> FOO
  def transformString(x: String, f: String => String): String = {
    f(x)
  }                                               //> transformString: (x: String, f: String => String)String
  transformString("foo", x => x.toUpperCase)      //> res3: String = FOO
}