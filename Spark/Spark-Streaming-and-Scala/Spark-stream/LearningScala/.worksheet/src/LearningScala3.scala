object LearningScala3 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(225); 
  ...
  transformInt(2, x => {val y = x * 2; y * y});System.out.println("""res2: Int = """ + $show(res$2));$skip(401); 
  ...
  def toupper(x: String): String = {x.toUpperCase};System.out.println("""toupper: (x: String)String""");$skip(33); 
  val foo_upper = toupper("foo");System.out.println("""foo_upper  : String = """ + $show(foo_upper ));$skip(21); 
  println(foo_upper);$skip(79); 
  def transformString(x: String, f: String => String): String = {
    f(x)
  };System.out.println("""transformString: (x: String, f: String => String)String""");$skip(45); val res$3 = 
  transformString("foo", x => x.toUpperCase);System.out.println("""res3: String = """ + $show(res$3))}
}
